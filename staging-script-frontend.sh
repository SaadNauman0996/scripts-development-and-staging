...
cd /home/saad/staging/Haussler_frontend || { echo "Failed to navigate to the project directory"; exit 1; }
...
if tmux has-session -t haussler-frontend-staging 2>/dev/null; then
    echo "Restarting the application in the existing tmux session..."
    tmux send-keys -t haussler-frontend-staging C-c  # Send Ctrl+C to stop the application
    tmux send-keys -t haussler-frontend-staging 'npm run dev -- --host 0.0.0.0 --port 8103' C-m  # Start the application on port 8103
else
    echo "Tmux session not found. Creating a new tmux session and starting the application..."
    tmux new-session -d -s haussler-frontend-staging
    tmux send-keys -t haussler-frontend-staging 'cd /home/saad/staging/Haussler_frontend && npm run dev -- --host 0.0.0.0 --port 8103' C-m
fi
