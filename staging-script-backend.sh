...
# Go to /home/saad/deploy directory
cd /home/saad/deploy/Haussler-backend/ || { echo "Failed to navigate to /home/saad/deploy/Haussler-backend"; exit 1; }
...
if tmux has-session -t haussler-backend-staging 2>/dev/null; then
    ...
    tmux new-session -d -s haussler-backend-staging
    tmux send-keys -t haussler-backend-staging 'cd /home/saad/staging/Haussler-backend && source venv/bin/activate && python manage.py runserver 0.0.0.0:8101' C-m
fi
